...
public class GraphQLQueryBuilder : GraphQLBuilder
{
    ...
    private void AppendValue(StringBuilder builder, object value)
    {
        switch (value)
        {
            ...
            case FilterObject filterObject:
                AppendFilterObject(builder, filterObject);
                break;
            case FilterArray filterArray:
                AppendFilterArray(builder, filterArray);
                break;
            case string:
    ...
    private static bool HasFilterOrArguments(GraphQLObject queryObject)
    {
        return queryObject.Arguments is not null || queryObject.WhereFilter is not null;
    }
    ...
    private void AppendFilterObject(StringBuilder builder, FilterObject filter)
    {
        builder.Append("{ ");
        ...
        foreach (var entry in filter.Entries)
        {
            ...
            AppendCamelCase(builder, entry.Key);
            builder.Append(": ");
    ...
    private void AppendFilterArray(StringBuilder builder, FilterArray array)
    {
        builder.Append("[ ");
    ...
    private void AppendFields(StringBuilder builder, List<GraphQLQueryObjectField> fields)
    {
        foreach (var field in fields)
        {
            ...
            if (field.HasFields)
            {
                ...
                if (HasFilterOrArguments(field))
                {
                    builder.Append(" ");
                    AppendObjectArguments(builder, field);
                }
