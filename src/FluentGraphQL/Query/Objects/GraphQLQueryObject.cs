...
public class GraphQLQueryObject<T> : GraphQLQueryObject
{
    ...
    /// <summary>
    /// Adds a type-safe filter to the <c>where</c> argument, e.g. <c>Where(x =&gt; x.City == "Paris" &amp;&amp; x.Age &gt;= 18)</c>.
    /// Called more than once, the predicates are combined with AND. Use <see cref="GraphQL.Var{T}"/> for variable references.
    /// </summary>
    /// <param name="predicate">The filter predicate; it is read to build the filter, never compiled to run.</param>
    /// <returns>The same query object, to continue chaining.</returns>
    public GraphQLQueryObject<T> Where(Expression<Func<T, bool>> predicate)
    {
